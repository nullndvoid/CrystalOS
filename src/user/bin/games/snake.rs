use super::super::super::lib::geometry::{Direction, Position};
use crate::std::application::{Application, Error};
use crate::std::io::{Color, Display, KeyStroke, Stdin};
use crate::std::random::Random;
use crate::std::render::{ColorCode, ColouredChar, Dimensions, Frame, RenderError};
use crate::std::time;
...
impl Application for Game {
    ...
    async fn run(&mut self, args: Vec<String>) -> Result<(), Error> {
        ...
        self.render()
            .map_err(|_| Error::ApplicationError(String::from("failed to render game screen")))?;
        // run the game
        self.gameloop().await?;
        Ok(())
    }
...
impl Game {
    ...
    fn respawn_snakes(&mut self) {
        if let Gamemode::WithAI(_ai_count, snake_len, _poi) = self.gamemode {
            self.snakes
                .push(Snake::ai(self.snakes.len() + 1, snake_len));
        }
    }

    async fn gameloop(&mut self) -> Result<(), Error> {
        // main gameloop
                ...
                match res {
                    Status::Lost => {
                        if self.snakes[i].ai_controlled {
                            ...
                            self.render_end_screen().map_err(|_| {
                                Error::ApplicationError(String::from("failed to render end screen"))
                            })?;
    ...
    fn new_poi(&mut self) {
        self.pois.push(Position {
            x: Random::int(3, 76) as i64,
            y: Random::int(3, 21) as i64,
        });
    }

    fn replace_poi(&mut self, poi: &Position) {
        self.pois
            .remove(self.pois.iter().position(|p| p == poi).unwrap());
        self.new_poi();
    }
    ...
    fn render(&mut self) -> Result<(), RenderError> {
        ...
        for s in self.snakes.clone() {
            ...
            for point in s.tail.iter() {
                frame[24 - point.y as usize][point.x as usize] =
                    ColouredChar::coloured('▓', curr_colour);
            }
        }

        self.pois.iter().for_each(|poi| {
            frame[24 - poi.y as usize][poi.x as usize] =
                ColouredChar::coloured('o', ColorCode::new(Color::Red, Color::Black));
        });
        ...
        msg.chars().enumerate().for_each(|(i, c)| {
            if c != ' ' {
                frame[1][i] =
                    ColouredChar::coloured(c, ColorCode::new(Color::LightGreen, Color::Black))
            }
        });
    ...
    fn centre_text(dims: usize, text: String) -> String {
        // centres text in a string of whitespace of a given length
        let max_pad = dims / 2;
        let mut msg = String::new();
        msg.push_str(
            " ".repeat(max_pad - round_up(text.len() as f64 / 2.0))
                .as_str(),
        );
        msg.push_str(text.as_str());
        msg.push_str(
            " ".repeat(max_pad - round_down(text.len() as f64 / 2.0 + 0.51))
                .as_str(),
        );
        msg
    }
    ...
    fn render_end_screen(&mut self) -> Result<(), RenderError> {
        ...
        frame[10] = Game::centre_text(80, String::from("u lost"))
            .chars()
            .map(|c| ColouredChar::coloured(c, ColorCode::new(Color::Red, Color::Black)))
            .collect();
        frame[12] = Game::centre_text(80, String::from(format!("ur score was {}", self.score)))
            .chars()
            .map(|c| ColouredChar::coloured(c, ColorCode::new(Color::LightGreen, Color::Black)))
            .collect();
        frame[14] = Game::centre_text(80, String::from("L bozo"))
            .chars()
            .map(|c| ColouredChar::coloured(c, ColorCode::new(Color::Red, Color::Black)))
            .collect();
...
impl Snake {
    ...
    fn next(&mut self, tails: &Vec<Position>, points_of_interest: &Vec<Position>) -> Status {
        // returns (lose_condition, scored)
    ...
    fn lose_condition(&mut self, tails: &Vec<Position>) -> bool {
        // where tails includes the tail of every other snake
