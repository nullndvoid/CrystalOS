    ...
    fn render(&self) -> Result<Frame, RenderError> {
        ...
        for i in 0..25 {
            for j in 0..80 {
                ...
                frame[i][j] = ColouredChar { character, colour };
            }
        }
...
impl GameBoard {
    ...
    fn move_player(&mut self, playerid: u8, dx: i32, dy: i32) {
        if let Some(eliminated_id) =
            self.players[playerid as usize].move_player(dx, dy, &mut self.board)
        {
    ...
    fn run_fill_algorithm(&mut self) {
        ...
        for player in self.players.iter().filter(|p| p.alive) {
            ...
            for (dy, dx) in Self::DIRS.iter().take(4) {
            ...
            for y in 0..25 {
                for x in 0..80 {
                    if let Cell::Head(p, _) | Cell::Tail(p, _) | Cell::Solid(p, _) =
                        self.board[y][x]
                    {
                        if p == pid {
                            fill_grid[y][x] = true;
    ...
    fn clear_player_territory(&mut self, player_id: u8) {
        for y in 0..25 {
            for x in 0..80 {
                if let Cell::Head(pid, _) | Cell::Tail(pid, _) | Cell::Solid(pid, _) =
                    self.board[y][x]
                {
                    if pid == player_id {
